    ...
    with st.container(border=True):
        ...
        for retailer in leader_df['leader'].unique():
            nodes.append(Node(id=retailer, 
                             label=retailer, 
                             size=25, # <<< FIX 1: Reduced node size from 400 to 25
        ...
        for _, row in leader_df.iterrows():
            if abs(row['median_lag_days']) > 0:
                ...
                edges.append(Edge(source=source, 
                                 target=target, 
                                 label=lag_label,
                                 color='#808080', # Lighter color for edges
                                 font={'color': 'white', 'size': 14, 'strokeWidth': 0}, # strokeWidth=0 makes font clearer
                                 arrows='to')) # Ensure arrows are explicitly defined
        
        # --- FIX 2: Improved physics engine for better layout ---
